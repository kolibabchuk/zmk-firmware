...
static void ec11_apply_reading(struct ec11_data *drv_data, uint8_t a, uint8_t b) {
    ...
    if (strcmp(drv_cfg->model, "ec11") == 0) {
        if (a == drv_data->prev_a && b == drv_data->prev_b) {
            LOG_DBG("no state change: %u %u", a, b);
            return;
        }
        ...
        delta = (bwd1 == bwd2) ? (bwd1 ? -1 : +1) : 0; // delta == 0 implies missing states

    } else if (strcmp(drv_cfg->model, "r1") == 0) {
        if ((drv_data->prev_a == 1 && drv_data->prev_b == 1 && a == 0 && b == 0) ||
            (drv_data->prev_a == 1 && drv_data->prev_b == 0 && a == 1 && b == 1)) {
            delta = -1;
        } else if ((drv_data->prev_a == 1 && drv_data->prev_b == 1 && a == 1 && b == 0) ||
                   (drv_data->prev_a == 0 && drv_data->prev_b == 0 && a == 1 && b == 1)) {
            delta = 1;
        }
    }

    LOG_ERR("state %u%u -> %u%u delta:%d", drv_data->prev_a, drv_data->prev_b, a, b, delta);
...
static void ec11_period(struct k_timer *timer_id) {
    ...
    LOG_WRN("histogram 00:%u 01:%u 10:%u 11:%u", cnts[0], cnts[1], cnts[2], cnts[3]);
...
#define EC11_INST(n)                                                                               \
    ...
    static const struct ec11_config ec11_cfg_##n = {                                               \
        ...
        .model = DT_INST_PROP_OR(n, model, "ec11"),                                                \
    };                                                                                             \
